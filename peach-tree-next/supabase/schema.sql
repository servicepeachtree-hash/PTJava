...
create table if not exists public.products (
  ...
  storage_path text not null,   -- path inside the PRIVATE "products" storage bucket (the actual paid file)
  cover_image_url text,          -- public URL in the PUBLIC "product-images" bucket (marketing thumbnail)
...
-- profiles: you can read only your own row.
-- Deliberately NO update policy: there's no "edit your profile" feature built,
-- and a blanket "update your own row" policy would let anyone flip their own
-- is_admin/is_banned flags directly via the API, bypassing the app entirely.
-- If a profile-edit feature gets built later, scope its policy to specific
-- safe columns only (e.g. name), never the whole row.
