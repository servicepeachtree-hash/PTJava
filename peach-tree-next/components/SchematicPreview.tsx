...
export default function SchematicPreview({ path, getUrl, autoShow = false }: { path: string; getUrl: () => Promise<string>; autoShow?: boolean }) {
  ...
  async function load() {
    console.log('[SchematicPreview] path:', path, '| isBBModel:', isBBModel, '| isSchematic:', isSchematic);
    const url = await getUrl();
    console.log('[SchematicPreview] got signed url, fetching…');
    const res = await fetch(url);
    console.log('[SchematicPreview] fetch status:', res.status, res.headers.get('content-type'));
    ...
    if (isBBModel) {
      ...
      console.log('[SchematicPreview] downloaded bytes:', buf.byteLength);
