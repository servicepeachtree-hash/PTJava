      ...
      <a href={detailHref}>
        {bundle.cover_image_url ? (
          <div className="bundle-cover">
            <Image src={bundle.cover_image_url} alt={bundle.name} fill sizes="340px" quality={90} style={{ objectFit: 'contain', background: 'var(--surface-2)' }} />
          </div>
        ) : (
          <div className="bundle-collage">
            {products.slice(0, 4).map((p) => (
              <div key={p.id} className="bundle-collage-cell">
                {p.cover_image_url ? (
                  <Image src={p.cover_image_url} alt={p.name} fill sizes="180px" quality={90} style={{ objectFit: 'contain' }} />
                ) : (
