...
import { readCartIds, readBundleCartIds } from '@/lib/cart';

export default async function SiteNav({ active }: { active?: 'home' | 'store' | 'collections' }) {
  const supabase = supabaseServer();
  const { data: { user } } = await supabase.auth.getUser();
  const cartCount = readCartIds().length + readBundleCartIds().length;
    ...
    <header className="site">
      <div className="nav-inner">
        ...
        <nav className="primary">
          ...
          <a href="/collections" className={active === 'collections' ? 'active' : ''}>Collections</a>
          <a href="https://discord.gg/EA2Kq48XDP" target="_blank" rel="noopener noreferrer">Contact</a>
        </nav>
        <div className="nav-actions">
          <img src="/images/badge2.png" alt="Official Minecraft Partner" className="nav-partner-badge" />
          <a href="https://peachtree.games" target="_blank" rel="noopener noreferrer" className="bedrock-btn">
            Bedrock
          </a>
          {isAdmin && (
