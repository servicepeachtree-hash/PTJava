...
export default async function HomePage() {
  ...
  const [counts, bestSellers, newReleases] = await Promise.all([
    ...
    supabase.from('products').select('*').eq('is_active', true).order('created_at', { ascending: true }).limit(4),
    ...
    <div className="storefront-root">
      ...
      <section className="section wrap">
        <div className="section-head">
          <div><span className="eyebrow">Proven favorites</span><h2>Best sellers</h2></div>
          <a href="/store" className="view-all">View all →</a>
        </div>
        <div className="product-grid">
          {(bestSellers.data ?? []).map((p: any) => <ProductCard key={p.id} product={p} />)}
          {(!bestSellers.data || bestSellers.data.length === 0) && (
            <p className="muted">No products yet — add one from /admin/products.</p>
          )}
        </div>
      </section>
      ...
      <section className="section wrap">
        ...
        <div className="product-grid">
          {(newReleases.data ?? []).map((p: any) => <ProductCard key={p.id} product={p} badge="new" />)}
