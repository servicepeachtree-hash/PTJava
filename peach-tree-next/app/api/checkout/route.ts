...
export async function GET(req: Request) {
  ...
  const { data: productsRaw } = await supabase.from('products').select('*').in('id', ids).eq('is_active', true);
  if (!productsRaw || productsRaw.length === 0) return NextResponse.redirect(new URL('/cart', req.url));

  // Never charge for something already owned, even if it somehow ended up in the cart.
  ...
  const products = productsRaw.filter((p: any) => !ownedIds.has(p.id));

  if (products.length === 0) return NextResponse.redirect(new URL('/account/library', req.url));
