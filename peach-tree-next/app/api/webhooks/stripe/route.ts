...
export async function POST(req: Request) {
  ...
  // Only now — a real, confirmed payment — does the coupon's usage count go up.
  const couponDiscountId = session.metadata?.coupon_discount_id;
  if (couponDiscountId) {
    const { data: discount } = await admin.from('discounts').select('redemption_count').eq('id', couponDiscountId).maybeSingle();
    if (discount) {
      await admin.from('discounts').update({ redemption_count: discount.redemption_count + 1 }).eq('id', couponDiscountId);
    }
  }
