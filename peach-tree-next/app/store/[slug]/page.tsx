...
import { getOwnedPreviewUrl } from '@/app/(app)/account/library/actions';
      ...
      <div className="wrap" style={{ padding: '32px 0 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 40, alignItems: 'start' }}>
          ...
          <div>
            ...
            {product.schematic_path && (
              <div className="admin-card" style={{ marginBottom: 20 }}>
                <h2>{product.schematic_path.toLowerCase().endsWith('.bbmodel') ? 'Model Preview' : 'Schematic Preview'}</h2>
                {isOwned ? (
                  <SchematicPreview
                    path={product.schematic_path}
                    getUrl={getOwnedPreviewUrl.bind(null, product.id, product.schematic_path)}
                    autoShow
                  />
                ) : (
                  <p className="muted" style={{ fontSize: 13 }}>
                    This product includes a {product.schematic_path.toLowerCase().endsWith('.bbmodel') ? 'Blockbench model' : 'schematic'} file.
                    Preview it anytime from your Library after purchase.
                  </p>
                )}
              </div>
            )}
