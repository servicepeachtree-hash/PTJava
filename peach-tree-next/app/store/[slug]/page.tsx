      ...
      <div className="wrap" style={{ padding: '32px 0 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 40, alignItems: 'start' }}>
          <div>
            <div className="pcard-media" style={{ clipPath: 'none', borderRadius: 14, aspectRatio: '16/10' }}>
              {product.cover_image_url ? (
                ...
                <div className="ph" style={{ fontSize: 20 }}>{product.name}</div>
              )}
            </div>

            {embedUrl && (
              <div style={{ marginTop: 16, position: 'relative', paddingBottom: '56.25%', height: 0, borderRadius: 12, overflow: 'hidden' }}>
                <iframe
                  src={embedUrl}
            ...
            {(product.media_urls ?? []).length > 0 && (
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>
                {product.media_urls.map((u: string, i: number) => (
                  <img key={i} src={u} alt="" style={{ width: 110, height: 110, objectFit: 'cover', borderRadius: 8, border: '1px solid var(--border)' }} />
                ))}
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {product.description && (
              <div className="prose-display" dangerouslySetInnerHTML={{ __html: product.description }} />
            )}
          </div>
        </div>
        ...
        {upsell && (
          <div style={{ marginTop: 56 }}>
            ...
            <div className="product-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', maxWidth: 280 }}>
              <ProductCard product={upsell} discountPercent={automaticPercentFor(upsell.id, discounts)} />
            </div>
          </div>
        )}
      </div>
