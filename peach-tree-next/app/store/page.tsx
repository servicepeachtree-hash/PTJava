...
export default async function StorePage({
  ...
  let bundles: any[] = [];
  let bundleProductsMap = new Map<number, any>();
  if (!category && !q) {
    const { data: b } = await supabase.from('bundles').select('*').eq('is_active', true).order('sort_order', { ascending: true });
    bundles = b ?? [];
    const allIds = Array.from(new Set(bundles.flatMap((bd) => bd.product_ids ?? [])));
    if (allIds.length > 0) {
      const { data: bp } = await supabase.from('products').select('id, name, price_cents, cover_image_url').in('id', allIds);
      bundleProductsMap = new Map((bp ?? []).map((p: any) => [p.id, p]));
    }
  }
          ...
          {bundles.length > 0 && (
            <div style={{ marginBottom: 36 }}>
              <div className="section-head" style={{ marginBottom: 16 }}>
                <div><span className="eyebrow">Buy together and save</span><h2 style={{ fontSize: 20 }}>Bundles</h2></div>
              </div>
