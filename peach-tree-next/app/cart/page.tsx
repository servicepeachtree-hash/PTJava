...
import { loadActiveDiscounts, automaticPercentFor, automaticPercentForBundle, couponPercentFor, couponPercentForBundle, applyPercent } from '@/lib/discounts';
...
export default async function CartPage() {
  ...
  const bundleLines = (bundlesRaw ?? []).map((b: any) => {
    ...
    const autoBundlePercent = automaticPercentForBundle(b.id, discounts);
    const couponBundlePercent = couponCode ? (couponPercentForBundle(b.id, couponCode, discounts) ?? 0) : 0;
    const effectivePercent = Math.max(b.discount_percent ?? 0, autoBundlePercent, couponBundlePercent);
    return { bundle: b, pricing: computeBundlePricing(bundleProducts, ownedIds, effectivePercent) };
  });
  ...
  const couponInvalid = couponCode
    && !lines.some((l) => couponPercentFor(l.product.id, couponCode, discounts) !== null)
    && !bundleLines.some((bl) => couponPercentForBundle(bl.bundle.id, couponCode, discounts) !== null);
        ...
        {isEmpty ? (
          ...
          <>
            ...
            <div className="admin-card">
              ...
              <p className="muted" style={{ fontSize: 11, marginTop: 10 }}>Coupon codes work on individual products and bundles — whichever the code was created for.</p>
            </div>
