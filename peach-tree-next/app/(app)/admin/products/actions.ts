...
export type ProductInput = {
  ...
  schematicPath: string | null;   // already uploaded, private "products" bucket
...
export async function createProduct(input: ProductInput) {
  ...
  const { error } = await admin.from('products').insert({
    ...
    schematic_path: input.schematicPath,
    media_urls: input.mediaUrls,
    tags: input.tags,
    upsell_product_id: input.upsellProductId,
...
export type ProductUpdateInput = Omit<ProductInput, 'mainFilePath' | 'coverImageUrl' | 'schematicPath' | 'mediaUrls'> & {
  id: number;
  isActive: boolean;
  mainFilePath: string | null;     // null = keep existing file
  coverImageUrl: string | null;    // null = keep existing
  schematicPath: string | null;    // null = keep existing
...
export async function updateProduct(input: ProductUpdateInput) {
  ...
  if (input.schematicPath) update.schematic_path = input.schematicPath;
