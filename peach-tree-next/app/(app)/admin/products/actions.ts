...
export async function createProduct(formData: FormData) {
  ...
  const productVersion = String(formData.get('product_version') || '1.0').trim();
  ...
  const { error: insertErr } = await admin.from('products').insert({
    slug, name, product_version: productVersion, description, category,
...
export async function updateProduct(formData: FormData) {
  ...
  const update: Record<string, any> = {
    name, slug, product_version: productVersion, description, category,
    price_cents: Math.round(price * 100),
