...
export async function createBundle(input: BundleInput) {
  ...
  await logAdminAction('create_bundle', input.slug, { name: input.name, product_ids: input.productIds });
...
export async function updateBundle(id: number, input: BundleInput) {
  ...
  await logAdminAction('update_bundle', String(id), { name: input.name });
  redirect('/admin/bundles?success=1');
}
