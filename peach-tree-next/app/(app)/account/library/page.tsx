...
import { getOwnedPreviewUrl } from './actions';
...
export default async function LibraryPage() {
  ...
  const { data: items } = await supabase
    .from('entitlements')
    .select('granted_at, product:products(id, name, category, schematic_path)')
    .eq('user_id', user.id)
    .eq('revoked', false)
          ...
          <tbody>
            ...
            {items.map((it: any, i: number) => (
              <tr key={i}>
                <td>
                  {it.product?.name}
                  {it.product?.schematic_path && (
                    <SchematicPreview
                      path={it.product.schematic_path}
                      getUrl={getOwnedPreviewUrl.bind(null, it.product.id, it.product.schematic_path)}
                    />
                  )}
                </td>
