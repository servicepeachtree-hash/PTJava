...
export async function requestPasswordReset(formData: FormData) {
  ...
  // This was the missing piece: without reading the Turnstile response and
  // forwarding it as captchaToken below, Supabase's Auth server rejects every
  // request with "captcha protection: request disallowed (no captcha_token
  // found)" as soon as CAPTCHA protection is enabled on the project.
  ...
  // Supabase's resetPasswordForEmail() doesn't leak whether the email exists
  // (it returns success either way) — so any error surfaced here is a real
  // technical failure (bad/missing captcha, malformed input), not an
