...
## Update: full admin suite + design fixes

This version adds: a real admin dashboard with sidebar nav, a Purchases page,
Discounts (sitewide or per-product sales, with or without a coupon code),
Reviews management, an enhanced Members page (see what each customer owns,
send password resets), and a full Product create/edit form (tags, upsell,
YouTube trailer, thumbnail, media gallery, optional .schem/.bbmodel upload).
Also fixes the missing underline reset and restores the geometric background
on the storefront.

**To update an existing deployment:**

1. **Run the migration** — Supabase → SQL Editor → paste and run
   `supabase/migration-003-admin-suite.sql`
2. **Password reset needs one more Supabase setting** — Authentication →
   URL Configuration → add `https://your-domain.vercel.app/reset-password`
   to **Redirect URLs** (otherwise the reset link Supabase emails won't be
   allowed to land on that page)
3. **Re-upload the code** — same as before: open your repo on GitHub,
   navigate into the folder that directly contains `app`, `lib`, etc.
   (not a wrapper folder — if in doubt, delete everything and re-upload
   the *contents* of the unzipped folder, not the folder itself), click
   **Add file → Upload files**, select everything, commit. Vercel redeploys
   automatically.

**Known gap:** discount percentages aren't applied to prices shown on the
storefront or charged at checkout yet — the Discounts page creates and
manages them, but the price math isn't wired into `/store`, the homepage
cards, or `/api/checkout` yet. Flag when you want that connected.
