...
## Update: real storefront connected to the database

This version adds the actual homepage and store page — pulling live products
from Supabase instead of the static mockup — plus an optional cover image
per product. If you already have this deployed, do these 3 things:

1. **Run the migration** — Supabase → SQL Editor → paste and run the contents
   of `supabase/migration-002-cover-image.sql`
2. **Create a second storage bucket** — Supabase → Storage → New bucket →
   name it exactly `product-images` → this one should be **Public** (toggle
   ON, unlike the `products` bucket). This holds marketing thumbnails only —
   the actual paid files still live in the private `products` bucket.
3. **Re-upload the code** — on GitHub, open your repo, navigate into the
   `peach-tree-next` folder (the one already containing `app`, `lib`, etc.),
   click **Add file → Upload files**, select everything from your freshly
   unzipped folder, and upload — GitHub will ask to confirm replacing the
   files that changed. Commit. Vercel redeploys automatically within a minute.
