...
export async function createProduct(input: ProductInput) {
  ...
  await logAdminAction('create_product', input.slug, { name: input.name, price_cents: Math.round(input.price * 100) });
...
export async function updateProduct(input: ProductUpdateInput) {
  ...
  await logAdminAction('update_product', String(input.id), { name: input.name });
  redirect('/admin/products?success=1');
}
